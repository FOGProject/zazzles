...
namespace Zazzles.Core.Process
{
    public class ProcessHandler
    {
        private ILogger _logger;
        private IProcess _processAPI;
        private string _workingDir;
        private Settings.OSType _os;

        public ProcessHandler(string workingDir, IProcess processAPI, ILogger<ProcessHandler> logger)
        {
            _logger = logger;
        ...
        public ProcessHandler(IProcess processAPI, ILogger<ProcessHandler> logger)
        {
            _logger = logger;
            _processAPI = processAPI;

            using(_logger.BeginScope(nameof(ProcessHandler)))
            {
                _workingDir = Directory.GetCurrentDirectory();
                _logger.LogInformation("Set working dir to '{workingDir}'", _workingDir);
            }
        }
        ...
        public int RunLocalEXE(string file, string param, bool wait = true)
        {
            return RunLocalEXE(file, param, wait, out string[] stdout);
        }
        ...
        /// <param name="wait">Wait for the process to exit</param>
        /// <returns>The exit code of the process. Will be -1 if wait is false.</returns>
        public int RunLocalEXE(string file, string param, bool wait, out string[] stdout)
        {
            if (string.IsNullOrEmpty(file))
                throw new ArgumentException("File name must be provided!", nameof(file));

            return RunEXE(Path.Combine(_workingDir, file), param, wait, out stdout);
        }
        ...
        /// <param name="wait">Wait for the process to exit</param>
        /// <returns>The exit code of the process. Will be -1 if wait is false.</returns>
        public int RunEXE(string filePath, string param, bool wait = true)
        {
            return RunEXE(filePath, param, wait, out string[] stdout);
        }
        ...
        /// <param name="wait">Wait for the process to exit</param>
        /// <returns>The exit code of the process. Will be -1 if wait is false.</returns>
        public int RunEXE(string filePath, string param, bool wait, out string[] stdout)
        {
            // If the current OS is Windows, simply run the process
            if (_os == Settings.OSType.Windows)
        ...
        /// <param name="wait">Wait for the process to exit</param>
        /// <returns>The exit code of the process. Will be -1 if wait is false.</returns>
        public int Run(string filePath, string param, bool wait = true)
        {
            string[] stdout;
            return Run(filePath, param, wait, out stdout);
        }
        ...
        /// <param name="wait">Wait for the process to exit</param>
        /// <returns>The exit code of the process. Will be -1 if wait is false.</returns>
        public int Run(string filePath, string param, bool wait, out string[] stdout)
        {
            if (string.IsNullOrEmpty(filePath))
                throw new ArgumentException("File path must be provided!", nameof(filePath));
            if (param == null)
        ...
        public Process CreateImpersonatedClientEXE(string filePath, string param, string user)
        {
            if (_os == Settings.OSType.Windows)
            ...
            var fileName = "su";
            var arguments = "- " + user + " -c \"mono " + Path.Combine(_workingDir, filePath) + " " + param;
        ...
        /// </summary>
        /// <param name="name">The name of the process</param>
        public void KillAllEXE(string name)
        {
            if (string.IsNullOrEmpty(name))
                throw new ArgumentException("Process name must be provided!", nameof(name));
        ...
        public void KillEXE(string name)
        {
            if (string.IsNullOrEmpty(name))
                throw new ArgumentException("Process name must be provided!", nameof(name));
